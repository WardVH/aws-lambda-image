...
const WebpImageGenerator = require("./WebpImageGenerator");
...
class ImageProcessor {
    ...
    processImage(imageData, config) {
        ...
        if (config.exists("reduce")) {
            ...
            promise = promise
                ...
                .then(() => Promise.resolve(++processedImages))
                .then(() => this.execWebpGeneration(reduce, imageData))
                .then((image) => this.fileSystem.putObject(image, reduce))
                .then(() => Promise.resolve(++processedImages));
        }
    ...
    execWebpGeneration(option, imageData) {
        const webpGenerator = new WebpImageGenerator(option);

        return webpGenerator.exec(imageData);
    }
